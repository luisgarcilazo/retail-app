      ...
      if (result != undefined){
        ...
        if(cartStr == null){
          ...
          if(hasEnoughStock == true){
            const dialogRef2 = this.dialog.open(SuccessAddCartDialog);
          } else {
            const dialogRef2 = this.dialog.open(NotEnoughStockDialog);
          }
...
export class AddToCartDialog {
  constructor(
    public dialogRef: MatDialogRef<DisplayAllComponent>,
...
@Component({
  selector: 'sucess-add-cart-dialog',
  templateUrl: 'success-add-cart-dialog.html',
  standalone: true,
  imports: [MatDialogModule, MatButtonModule],
})
export class SuccessAddCartDialog {}

@Component({
  selector: 'd-all-not-enough-stock-dialog.',
