...
export class FruitsComponent implements OnInit {
  ...
  retrieveProducts(): void{
    this.productService.getAllProducts().subscribe((products) => {
      this.products = products.filter((a) => a.category == "Fruits").sort((a,b) => (a.name > b.name ? 1 : -1));
    })
  }
  ...
  addToCart(product: Product): void{
    ...
    const dialogRef = this.dialog.open(FruitsAddToCartDialog, {
      ...
      if (result != undefined){
        ...
        if(cartStr == null){
          ...
          if(hasEnoughStock == true){
            const dialogRef2 = this.dialog.open(FruitsSuccessAddCartDialog);
          } else {
            const dialogRef2 = this.dialog.open(NotEnoughStockDialog);
          }
...
@Component({
  selector: 'fruits-add-to-cart-dialog',
  templateUrl: 'fruits-add-to-cart-dialog.html',
  standalone: true,
  imports: [MatDialogModule, MatFormFieldModule, MatInputModule, FormsModule, MatButtonModule, MatSelectModule],
})
export class FruitsAddToCartDialog {
  constructor(
    public dialogRef: MatDialogRef<FruitsComponent>,
...
@Component({
  selector: 'fruits-success-add-cart-dialog',
  templateUrl: 'fruits-success-add-cart-dialog.html',
  standalone: true,
  imports: [MatDialogModule, MatButtonModule],
})
export class FruitsSuccessAddCartDialog {}

@Component({
  selector: 'fruits-not-enough-stock-dialog.',
